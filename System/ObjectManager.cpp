...
void ObjectManager::Draw()
{
	...
	{
		obj->RunProcess([&]() { return obj->PreDraw(); });
	}
	for (const auto& obj : m_gameObjects)
	{
		obj->RunProcess([&]() { return obj->Draw(); });
	}
	for (const auto& obj : m_gameObjects)
	{
		obj->RunProcess([&]() { return obj->PostDraw(); });
	}
}
...
void ObjectManager::UpdateGameObjects()
{
	...
	for (const auto& obj : m_gameObjects)
	{
		obj->RunProcess([&]() { return obj->PreUpdate(); });
	}
	for (const auto& obj : m_gameObjects)
	{
		obj->RunProcess([&]() { return obj->Update(); });
	}
	for (const auto& obj : m_gameObjects)
	{
		obj->RunProcess([&]() { return obj->PostUpdate(); });
	}
	m_isUpdatingGameObject = false;
...
void ObjectManager::CheckCollision()
{
	...
	{
		// 状態がアクティブなら衝突をチェック
		if (checkObjects.front()->GetState() == GameObject::State::EActive)
		{
			// 他のオブジェクトとの衝突を判定
			for (const auto& obj : checkObjects)
			{
				...
				// 状態がアクティブではないならスキップ
				if (obj->GetState() != GameObject::State::EActive) continue;
